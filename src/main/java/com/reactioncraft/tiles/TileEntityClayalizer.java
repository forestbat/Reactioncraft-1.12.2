...
public class TileEntityClayalizer extends TileEntityLockable implements ITickable, ISidedInventory
{
    ...
    {
        boolean flag = !stack.isEmpty() && stack.isItemEqual(itemStacks.get(index)) && ItemStack.areItemStackTagsEqual(stack, itemStacks.get(index));
...
//        this.furnaceItemStacks = new ArrayList<>(getSizeInventory());
        ...
        this.currentItemBurnTime = getItemBurnTime(itemStacks.get(1));
    ...
    public void update()
    {
        ...
        {
            ItemStack one= itemStacks.get(1),zero= itemStacks.get(0);
            if (this.isBurning() || !one.isEmpty() && !zero.isEmpty())
            {
                if (!this.isBurning() && this.canSmelt())
                {
                    ...
                    if (this.isBurning())
                    {
                        ...
                        if (!one.isEmpty())
                        {
                            ...
                            if (one.getCount()== 0)
                            {
                                this.itemStacks.set(1, one.getItem().getContainerItem(one));
                            }
                        }
                    }
                }
            ...
            {
                flag1 = true;
                BlockClayalizer.setState(this.isBurning(), this.world, this.pos);
            }
        }
